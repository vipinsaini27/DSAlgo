"""
Problem Description
Find the intersection of two sorted arrays. OR in other words, Given 2 sorted arrays, find all the elements which occur
in both the arrays.
...
The obvious brute-force solution is to scan through each element in A, and for each element in A, scan if that element 
exist in B.
...
hash each element in B to an array index (takes O(n) time).
...
Therefore, using a hash table is not a good general solution to this problem. Besides, using hash table DOES NOT require 
for the array to be sorted in the first place.
Here is the most important observation in order to solve this Both</a> arrays ARE sorted. This provides a very important 
clue. We must make full use of this information that they ARE in fact sorted.
...
The complexity is O(m+n) because in the worst case, there would be no intersection between the two arrays, and we need 
to increment the first index a total of m times and increment the second index a total of n times, which is a total of 
m+n times.
"""

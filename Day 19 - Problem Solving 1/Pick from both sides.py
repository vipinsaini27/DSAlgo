...
etc. You need to return the maximum possible sum of elements you can pick.
...
Maintain two arrays prefix[i] and suffix[i] where prefix[i] denotes sum of elements from index [0,i] and suffix[i] 
denotes sum of elements from index [i,N-1].
Now iterate from left and one by one pick elements from left for example: if you pick ‘a’ elements from left and 
...
class Solution:
    ...
    def solve(self, A, B):
        ...
        i = 1
        while i < len(A):
            LS.append(LS[i-1] + A[i])
            i += 1

        i = len(A) - 2
        while i > -1:
            RS.appendleft(RS[0] + A[i])
            i -= 1
